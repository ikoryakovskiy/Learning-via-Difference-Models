...
def train(args, ddpg, actor, critic, counter=None, diff_model=None, model=None):
    ...
    with tf.Session(graph=diff_model,config=tf.ConfigProto(gpu_options=gpu_options)) as diff_sess:
        ...
        with tf.Session(graph=ddpg,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            # Start the GRL code
            # code = Popen(['../notebooks/grl/build/grld', args])
            ...
            print save_counter
	    print "Noise sigma:", ddpg_params.ou_sigma
            ...
            episode_count = 0
            ou_sigma = ddpg_params.ou_sigma
            ou_theta = ddpg_params.ou_theta
	    ou_mu = OU_MU
            ...
            while True:
                        ...
                        if test_agent:
                            if terminal == 2:
                                check = replay_buffer.transitions_buffer_add(np.reshape(state_old, (STATE_DIMS,)),
                                                               ...
                                                               reward, True, np.reshape(state, (STATE_DIMS,)), diff_state_variance)
                            else:
                                check = replay_buffer.transitions_buffer_add(np.reshape(state_old, (STATE_DIMS,)),
                                                               np.reshape(computed_action, (actor.a_dim,)),
                                                               reward, False, np.reshape(state, (STATE_DIMS,)), diff_state_variance)
                        else:
                            if terminal == 2:
                                check = replay_buffer.replay_buffer_add(np.reshape(obs_old, (actor.s_dim,)),
                                                                        np.reshape(computed_action, (actor.a_dim,)),
                                                                        reward, True, np.reshape(obs, (actor.s_dim,)), diff_state_variance)
                            else:
                                check = replay_buffer.replay_buffer_add(np.reshape(obs_old, (actor.s_dim,)),
                                                                        np.reshape(computed_action, (actor.a_dim,)),
                                                                        reward, False, np.reshape(obs, (actor.s_dim,)), diff_state_variance)
                    if check:
                    ...
                    if episode_start and episode_count != 1:
                        ...
                        #        successful_episodes += 1
                            if test_reward > max_test_reward:
                                ddpg_params.reward = test_reward
                ...
                if ddpg_params.test_run_on_model:
...
def start(args, counter=None):
    ...
    with tf.Graph().as_default() as ddpg:
        ...
        critic = CriticNetwork(OBSERVATION_DIMS, ACTION_DIMS, critic_learning_rate, TAU,
