...
def main():
    ...
    if args.cores:
        ...
        args.cores = min(multiprocessing.cpu_count(), 10)
...
#    yaml.add_constructor(_mapping_tag, dict_constructor)
    ...
    list_of_new_cfgs = rl_run_zmqagent(["/leo_drl_server/leo_rbdl_zmq_drl_diff_rate.yaml"], range(runs))
    ...
    for cfg in list_of_cfgs:
        ...
        #    conf['experiment']['environment']['xml'] = "../grl/addons/cfg/leo/xm430_210_vc_leo_walk_ankle_bound.xml"
            ...
            if "difference_model" in conf['experiment']:
                conf['experiment']['difference_model'] = run+1
            write_cfg(list_of_new_cfgs[-1], conf)
...
def remove_viz(conf):
    ...
    if "model" in conf['experiment']['environment']:
        if "animation" in conf['experiment']['environment']['model']:
            conf['experiment']['environment']['model']['animation'] = "nope"
